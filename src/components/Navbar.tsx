...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#0A0A09]/95 backdrop-blur-sm border-b border-[#2A2A28] h-16 flex items-center">
      <div className="container mx-auto flex items-center justify-between px-6">
        ...
        <Link to="/" className="flex items-center gap-2.5 group flex-shrink-0">
          ...
          <span className="hidden sm:flex items-baseline gap-1.5">
            <span className="font-display font-bold text-[#F5F1E8] text-[17px] tracking-tight leading-none">Source</span>
            <span className="font-body font-light text-[#8A8B80] text-[13px] tracking-wide">Harvard</span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          {user ? (
            <>
              ...
              {role === "admin" && (
                <Link to="/admin" className="md:hidden p-2 text-crimson hover:text-crimson-dim transition-colors">
                  <ShieldAlert className="w-4 h-4" />
                </Link>
              )}
              {role === "founder" && (
                <Link to="/submit">
                  <button className="inline-flex items-center gap-1.5 h-9 px-4 bg-crimson text-white text-[13px] font-body font-medium rounded-sm hover:bg-crimson-dim transition-colors duration-150">
            ...
            <>
              ...
              <Link to="/signup">
                <button className="h-9 px-5 bg-crimson text-white text-[13px] font-body font-medium rounded-sm hover:bg-crimson-dim transition-colors duration-150">
                  Join
                </button>
              </Link>
            </>
